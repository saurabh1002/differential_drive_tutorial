...
namespace gazebo {
    ...
    class DiffDrivePlugin : public ModelPlugin {	// Inherits the ModelPlugin Class. Various types of Plugins can be found here: /usr/gazebo-<VERSION>/gazebo/common/Plugins.hh
        ...
        public: void GetJointsFromModel()
        {
            ...
            this->LeftWheel = this->model->GetJoint("left_wheel");		// The param passed to the function should be same as the name given in the sdf file.
            this->RightWheel = this->model->GetJoint("right_wheel");
...
// There are many such plugin headers available for various elements like links, joints, sensors, etc. They can be found here: /usr/gazebo-<VERSION>/gazebo/
